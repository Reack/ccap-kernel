...
不同於傳統 AI 工具動輒讀取並複寫整個檔案，CCAP 提倡 **「漸進式精確度」** 工作流，旨在將每一顆 Token 都花在刀口上：

1.  **全局導航 (Global Navigation)**：AI 首先讀取體積僅為原始碼 1.8% 的「語義地圖」，建立對全系統拓樸的初步認知。
2.  **漸進鎖定 (Progressive Lock-on)**：AI 根據地圖中的幾何引力與符號特徵，快速識別出受影響的「語義室 (Room)」或特定符號，無需讀取無關檔案。
3.  **精確讀取 (Minimalist Read)**：僅針對目標符號進行「片段式讀取」，將 Context 消耗降至最低。
4.  **手術級更新 (Surgical Patching)**：透過 `ccap patch` 指令，僅更新發生變動的字元座標，徹底終結「全檔案複寫」導致的冗餘消耗與語義丟失。

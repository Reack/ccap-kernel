...
## 第十五章：美學通道與 AI 潤色協定 (Aesthetic Enrichment Protocol)

為平衡「極致省錢」與「高品質文檔」的需求，CSK 提供受控的 AI 語義增強通道。

### 15.1 語義範本注入 (Semantic Enrichment)
*   **機制**: 允許使用者對特定模組或全專案執行 `--ai-enrich`。
*   **對齊模式**: AI 僅讀取 ST-AAAK 高熵電報與特徵向量，嚴禁接觸原始碼。
*   **效益**: 用不到 3% 的 Token 預算，產出具備業務感、排版精美的「架構故事」。

### 15.2 全局戰略合成 (Global Strategic Synthesis)
*   **動作**: 匯整 1M+ 行專案的所有語義室摘要，由 AI 進行跨分區的邏輯推演。
*   **產出**: 產出「專案靈魂描述 (Project Soul)」，自動注入 Wiki 首頁作為高階導航指引。
*   **版本鎖定**: 所有 AI 產出的潤色內容必須標註 `[STORY_SYNC]` 時間戳與 Git Commit ID，確保語義不隨時間腐爛。
